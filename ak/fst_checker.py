        ...
        for token in doc:
            ...
            if token.pos_ in ["VERB", "ADJ", "NOUN", "X"]:
                ...
                for index in range(0, len(list_of_parses)):
                    ...
                    parse_pos = "X"  # Set pos of parse equal to X if parse doesn't have it
                ...
                if len(new_parse_list) > 0:
                    #If the pos is NOUN and there are multiple likely candidates, avoid the one with PredSuff* in it (as it is highly unlikely)
                    if token.pos_ == "NOUN":
                        for parse in new_parse_list:
                            if len({"PredSuffGen","PredSuffNum","PredSuffPer"}.intersection(parse.keys())) == 0:
                                no_pred_list.append(parse)

                        #If one candidate among the multiple top ranking ones does not have PredSuff* in it, chose that one
                        if len(no_pred_list) > 0:
                            likely_analysis = no_pred_list[0]
                        #Else just pick one of the top candidates with PredSuff*
                        else:
                ...
                #If the likely analysis is empty, revert to original suggested analysis
                if likely_analysis == {}:
                    likely_analysis = token_morph_dict

                #Make deep copy of likely_analysis so that we can pop off POS key and value from it (if they exist) and set that as
                # the morph for the token. The original parse from the dialect dictionary will not be effected.
                final_analysis = copy.deepcopy(likely_analysis)

                if "POS" in final_analysis.keys():
                    final_analysis.pop("POS")

                print("final_analysis")
                print(final_analysis)

                token.set_morph(final_analysis)
