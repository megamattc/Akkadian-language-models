...
oraccVolume = 'barutu' #Short name of oracc project
oraccProject = 'cams/' + oraccVolume #Full name of oracc project
...
texts_translit = words_df.groupby([words_df['id_text'], words_df['id_line'], words_df['label']]).agg({
        'form': ' '.join,
    }).reset_index()
...
for PNum in PNumList:
    savefile = f'{PNum}.txt'
    with open(f'output_trans/{savefile}', 'w', encoding="utf-8") as w:
...
texts_norm = words_df.groupby([words_df['id_text'], words_df['id_line'], words_df['label']]).agg({
        'norm1': ' '.join,
    }).reset_index()
