...
def convert_pnum(pnum : str) -> str:

    url = ""

    #If we have a Pnum and not Qnum, we know it has a certain page in cdli
    if re.search(r'P[0-9]{6}',pnum):
        pnum2 = re.findall(r'P[0-9]{6}',pnum)[0]
        url = "https://cdli.mpiwg-berlin.mpg.de/" + pnum2

    #Another for the Qnums
    elif re.search(r'Q[0-9]{6}',pnum):
        pnum2 = re.findall(r'Q[0-9]{6}',pnum)[0]
        url = "https://cdli.mpiwg-berlin.mpg.de/artifacts/composites-score/" + pnum2
...
def find_form(lexeme,form : str, upos: str, xpos: str, morph : str,colu:str, lin_num:str,sur:str) -> dict:
    ...
    for form_dic in lexeme.forms.get_json():
        ...
        if akkadian_in_latin_script_misx in form_dic["representations"]:
            ...
            if form == form_value and set(convert_morphology(upos, xpos, morph)) == set(grammatical_features):
                ...
                l = "" #Line num
                s = "" #Surface
                pnl = "" #cdli address of pnum

                claims_dict = form_dic["claims"]

                if attested_in in claims_dict.keys():
                    pnum_link_dict = claims_dict[attested_in][0] #Assume only one object with property pnum_link
                    #If the attested source is CDLI:
                    if pnum_link_dict["mainsnak"]["datavalue"]["value"]["id"] == cdli_qnum:
                        pnum_link_quals = pnum_link_dict["qualifiers"]

                        if line_num in pnum_link_quals.keys():
                            line_num_dict = pnum_link_quals[line_num][0]
                            l = line_num_dict["datavalue"]["value"]

                        if column in pnum_link_quals.keys():
                            col_dict = pnum_link_quals[column][0]
                            c = col_dict["datavalue"]["value"]["id"]

                        if hassurface in pnum_link_quals.keys():
                            s_dict = pnum_link_quals[hassurface][0]
                            s = s_dict["datavalue"]["value"]["id"]

                        if described_at_url in pnum_link_quals.keys():
                            #There may be multiple url's given as qualifiers. In particular, cdli and oracc
                            for d in pnum_link_quals[described_at_url]:
                                if "cdli" in d["datavalue"]["value"]:
                                    pnl = d["datavalue"]["value"]



                        #IF all fields match, return form_dict that matches candidate
                        if [c, l, s] == [colu, lin_num, sur]:
...
for row in reader:
    ...
    pnum_link = convert_pnum(row["pnum"]) #URI for text on cdli
    ...
    project_url = "" # Oracc project url based on Pnum

    #Try to get Oracc project name based on Pnum
    if row["pnum"] in project_file_dict.keys():
        project_url = project_file_dict[row["pnum"]]
    ...
    if "-" in form or "}" in form or "{" in form:
        print("Form may be a fragment. Marking as problem and skipping.")
        row["problem"] = "x"
        counter = counter + 1
        writer.writerow(row)
        continue
    ...
    if re.search(r'(?<=VerbStem=)([A-Za-zŠ]{1,3})',row["morphology"]):
        ...
        print(re.findall(r'(?<=VerbStem=)([A-Za-zŠ]+)\|{0,1}',row["morphology"]))
    ...
    # If we have an LID, get the wiki lexeme associated with it
        ...
        if not find_form(lexeme, form, upos, xpos, row["morphology"],colu,lin_num,sur):
            ...
            c = Claim()
            c.update(datatypes.Item(prop_nr=attested_in, value=cdli_qnum))

            if pnum_link != "":
                c.qualifiers.set([datatypes.ExternalID(prop_nr=described_at_url, value=pnum_link)])
                #f.claims.add(datatypes.ExternalID(prop_nr=ext_url, value=pnum_dict["url"]))

            if label["surface"] != "":
                c.qualifiers.set([datatypes.Item(prop_nr=hassurface, value=label["surface"])])
                #f.claims.add(datatypes.Item(prop_nr=hassurface, value=label["surface"]))
            if label["column"] != "":
                c.qualifiers.set([datatypes.ExternalID(prop_nr=column, value=label["column"])])
                #f.claims.add(datatypes.ExternalID(prop_nr=column, value=label["column"]))
            if label["line_num"] != "":
                c.qualifiers.set([datatypes.ExternalID(prop_nr=line_num, value=label["line_num"])])
                #f.claims.add(datatypes.ExternalID(prop_nr=line_num, value=label["line_num"]))

            if project_url != "":
                c.qualifiers.set([datatypes.ExternalID(prop_nr=described_at_url, value=project_url)])
    ...
    ff = find_form(result,form,upos,xpos,row["morphology"],colu,lin_num,sur)
    print("ff:")
    print(ff)
