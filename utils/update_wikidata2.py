...
SELECT
  ?lexeme ?lemma
  ?lexical_category ?lexical_categoryLabel ?root
WITH {
  SELECT ?lexeme ?lemma ?lexical_category ?root WHERE {
    ?lexeme a ontolex:LexicalEntry ;
            dct:language wd:Q35518 ; 
            wikibase:lemma ?lemma .
    OPTIONAL {
      ?lexeme wdt:P5920 ?root .
    }
...
for result in results["results"]["bindings"]:
    ...
    entry.update({lemma:{"lid":lid,"lexcat":lexcat}})

    if "root" in result.keys():
        root_value = result["root"]["value"]
        root = root_value.split("/")[-1]
        entry[lemma].update({"root":root})


    #print(result)
    wikidata_lexemes.update(entry)
...
for row in reader:
    ...
    if row["Root"] != "":
        root = row["Root"]
        lid = wikidata_lexemes[root]["lid"] #We know root triple will be in the lexemes dictionary, so get its QID
        print("Trying to add root " + root)

        lexeme.claims.add(datatypes.Item(prop_nr=has_root, value=lid),
    ...
    if row["CDA_page"] != "":
        ...
        for cda_claim in cda_claims:
            ...
            if cda_claim.mainsnak.property_number == described_by_source and cda_claim.mainsnak.datavalue["value"]["id"] == cda:
                ...
                if row["CDA_numeral"] != "":
                    citation_cda = row["term"] + " " + row["CDA_numeral"]
                    cda_claim.qualifiers.set([datatypes.ExternalID(prop_nr=subject_named_as, value=citation_cda)])
    ...
    if row["CAD_page"] != "":
        ...
        for cda_claim in cda_claims:
            ...
            if cda_claim.mainsnak.property_number == described_by_source and cda_claim.mainsnak.datavalue["value"]["id"] == cad:
                ...
                # If there is a Roman numeral distinction (I/II/etc...), add it
                if row["CAD_letter"] != "":
                    citation_cad = row["term"] + " " + row["CAD_letter"]
                    cda_claim.qualifiers.set([datatypes.ExternalID(prop_nr=subject_named_as, value=citation_cad)])
    ...
    if row["logogram"] != "":
        ...
        lexeme.lemmas.set(language=akkadian_in_cuneiform_script_misx, value=logogram_lemma_converted,action_if_exists=ActionIfExists.KEEP)
        ...
        for i in range(0, len(logogram_array)):
            ...
            logogram_lemma_converted = convert_logogram(logogram_lemma, nuolenna_dict)
            print("Processing cuneiform form from spreadsheet: " + logogram_lemma_converted)
            ...
            f.claims.add(datatypes.Item(prop_nr=instance_of, value=paradigm),action_if_exists=ActionIfExists.APPEND_OR_REPLACE)
            print("(Logogram) Length of form json is " + str(len(lexeme.forms.get_json())))

            found = False
            for form_dic in lexeme.forms.get_json():
                #print("lexeme.forms.get_json()")
                #print(lexeme.forms.get_json())
                if akkadian_in_cuneiform_script_misx in form_dic["representations"]:
                    form_value = form_dic["representations"][akkadian_in_cuneiform_script_misx]["value"]
                    if logogram_lemma_converted == form_value:
                        print("(Logogram) Found match between cuneiform forms")
                        found = True
                        break


            if found == False:
                print("Adding logogram to forms")
                lexeme.forms.add(f)
        ...
        for i in range(0, len(alternate_array)):
            ...
            for form_dic in lexeme.forms.get_json():
                ...
                if akkadian_in_latin_script_misx in form_dic["representations"]:
                    ...
                    if alternate_lemma == form_value:
                        print("(Alternate spelling) Found match between latin forms")
                        found = True
                        break


            if found == False:
                print("Adding alternate spelling to forms")
                lexeme.forms.add(f)



    # We need to add the lexemeLabel to the list of forms for consistency's sake,
    print("Adding lexemelabel = " + lexemelabel + " to forms")
    f = Form()
    ...
    print("New form f to be added:")
    print(f)
    print("Lexeme.forms")
    print(lexeme.forms)
    #Checking if LID (and hence lexeme) already exists
    if row["LID"] != "":
        print("LID is present")
        print("Length of form json is " + str(len(lexeme.forms.get_json())))
        j = lexeme.forms.get_json()
        for k in j:
            print("Dictionary: ")
            print(k)

        found = False #Keeps track of whether we find a match with lexemelabel among forms
        #Go through list of dictionaries representing contents of forms
        for form_dic in lexeme.forms.get_json():
            #print("lexeme.forms.get_json()")
            #print(lexeme.forms.get_json())
            #form_key = row["LID"] + "-F" + str(form_index+1)
            if akkadian_in_latin_script_misx in form_dic["representations"]:
                form_value = form_dic["representations"][akkadian_in_latin_script_misx]["value"]
                if lexemelabel == form_value:
                    print("(Lexemelabel) Found match between Lexemelabel and form")
                    found = True
                    break


        if found == False:
            print("Lexeme has LID, adding lexemelabel to forms")
            lexeme.forms.add(f)
    else:
        print("Lexeme has no LID, adding lexemelabel to forms")
        lexeme.forms.add(f)
    ...
    if row["assyrian_dialect"] != "":
        ...
        for i in range(0, len(assyrian_array)):
            ...
            found = False  # Keeps track of whether we find a match among forms
            # Go through list of dictionaries representing contents of forms
            for form_dic in lexeme.forms.get_json():
                # print("lexeme.forms.get_json()")
                # print(lexeme.forms.get_json())
    ...
    for sense in sense_array:
        if sense != "":
            ...
            print("lexeme.senses")
            print(lexeme.senses)
            for lsense in lexeme.senses.get_json():
                #print("lsense:")
                #print(lsense)
                try:
                    egloss = lsense["glosses"]["en"]["value"]
                    #See if there is an English sense
                    if egloss == sense:
                        print("Found identical senses with " + egloss)
                        found = True
                except:
                    print("Could not find english gloss for sense:")
                    print(lsense)
